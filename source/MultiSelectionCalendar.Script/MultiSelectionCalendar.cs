	...
	public class MultiSelectionCalendar
	{
		...
		public static List<Range> GetDateRangesFromString(string timeRanges)
		{
			...
			foreach (var part in timeRanges.Split(';'))
			{
				...
				if (dates.Length == 2)
				{
					var start = DateTime.ParseExact(dates[0], "dd.MM.yyyy");
					var end = DateTime.ParseExact(dates[1], "dd.MM.yyyy");
		...
		public static string GetStringFromDateRanges(List<Range> ranges)
		{
			...
			foreach (var range in ranges)
			{
				...
				buf.Append(":");
				buf.Append(range.Start.Format("dd.MM.yyyy"));
				buf.Append("-");
				buf.Append(range.End.Format("dd.MM.yyyy"));
			}
			return buf.ToString();
		}
